...
QList<V3DLONG> DFS(QVector< QVector<V3DLONG> > neighbors, V3DLONG newrootid, V3DLONG siz, QHash<V3DLONG, NeuronSWC>& LUT_newid_to_node, bool& hasSoma, XYZ somaCoordinate)
{
    // siz: size of the whole neuronlist
    // The neuronlist may include multiple components
    // A component is a connected tree
    ...
    pstack.push(newrootid);
    neworder.append(newrootid);
    if(distance(LUT_newid_to_node[newrootid].x, somaCoordinate.x, LUT_newid_to_node[newrootid].y, somaCoordinate.y,
                 LUT_newid_to_node[newrootid].z, somaCoordinate.z) < 1 && somaCoordinate.x != 0){
        hasSoma = true;
    }
    ...
    while(!pstack.isEmpty()){
        ...
        for(it=cur_neighbors.begin(); it!=cur_neighbors.end(); ++it)
        {
            if(visited.at(*it)==0)
            {
                ...
                visited[*it]=1;
                neworder.append(*it);
                if(distance(LUT_newid_to_node[*it].x, somaCoordinate.x, LUT_newid_to_node[*it].y, somaCoordinate.y,
                             LUT_newid_to_node[*it].z, somaCoordinate.z) < 1 && somaCoordinate.x != 0){
                    hasSoma = true;
                }
                break;
            }
...
bool SortSWCAndDetectLoop(QList<NeuronSWC> & neurons, V_NeuronSWC_list segments, QList<NeuronSWC> & result, vector<CellAPO>& loopMarkers, V3DLONG newrootid)
{
    ...
    while (sorted_size <siz)
    {
        ...
        specificPathNums[new_root]=-1;
        cur_neworder = DFS(neighbors, new_root, siz, specificPathNums);
        //        qDebug()<<QString("cur_neworder=%1").arg(cur_neworder.size());
        sorted_size += cur_neworder.size();
        neworder.append(cur_neworder);
        for(int i=0; i<cur_neworder.size(); i++){
...
set<string> getTreeMarkerPoints(QList<NeuronSWC> & neurons, XYZ somaCoordinate){
    // modified by Sujun
    // add empty file judgement
    ...
    map<int, bool> groupIndex2HasSomaMap;
    QList<V3DLONG> output_newroot_list;
    output_newroot_list.append(root);
    ...
    // generate the 1st sorted tree.
    bool hasSoma = false;
    cur_neworder= DFS(neighbors, root, siz, LUT_newid_to_node, hasSoma, somaCoordinate);
    ...
    groupIndex2HasSomaMap[cur_group] = hasSoma;
    ...
    while (sorted_size < siz)
    {
        ...
        hasSoma = false;
        cur_neworder = DFS(neighbors, new_root, siz, LUT_newid_to_node, hasSoma, somaCoordinate);
    ...
    qDebug()<<"Number of components before making connections"<<cur_group;
    if(somaCoordinate.x != 0){
        for(int i = 1; i <= cur_group; i++){
            if(!groupIndex2HasSomaMap[i]){
...
{
    if (isFileLocked(fileSaveName.toStdString())) {
        std::cout << "File is locked by another program" << std::endl;
    } else {
